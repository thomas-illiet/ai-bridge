...
const props = defineProps<{ models: ModelCount[]; loading?: boolean }>()

const total = computed(() => props.models.reduce((s, m) => s + m.count, 0))
    ...
    <div v-else class="model-list">
      <div v-for="(m, i) in models" :key="m.model" class="model-row">
        <div class="model-meta">
          ...
          <span class="model-pct">{{ total > 0 ? Math.round(m.count / total * 100) : 0 }}%</span>
        </div>
        <div class="model-bar-track">
          <div
            class="model-bar-fill"
            :style="{ width: (total > 0 ? m.count / total * 100 : 0) + '%', background: color(i) }"
          />
        </div>
      </div>
    </div>
