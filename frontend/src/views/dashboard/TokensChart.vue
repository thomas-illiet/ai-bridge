...
const props = defineProps<{ days: DailyTokens[]; loading?: boolean }>()

const barMax = computed(() => Math.max(1, ...props.days.map(d => d.total)))

function barHeight(total: number) { return Math.round((total / barMax.value) * 120) }
...
<template>
  <div class="chart-card">
    <h2 class="chart-title">Tokens — Last 7 Days</h2>
    <div v-if="loading" class="chart-skeleton" />
    <div v-else class="bar-chart">
      <div v-for="day in days" :key="day.date" class="bar-col">
        <span class="bar-value">{{ day.total > 0 ? fmtNum(day.total) : '' }}</span>
        <div class="bar" :style="{ height: barHeight(day.total) + 'px' }" :class="{ 'bar-zero': day.total === 0 }" />
