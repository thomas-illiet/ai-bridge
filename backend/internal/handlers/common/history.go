...
const HistoryBaseSQL = `
	SELECT
		ai.id, ai.initiator_id, ai.provider, ai.provider_type, ai.model, ai.client_ip, ai.started_at, ai.ended_at,
...
func HistoryQuery(whereSQL string, whereArgs []any, page, pageSize int, sortBy, sortDir string) ([]InterceptionRow, int64, error) {
	...
	dataSQL := HistoryBaseSQL + whereSQL +
		` GROUP BY ai.id, ai.initiator_id, ai.provider, ai.provider_type, ai.model, ai.client_ip, ai.started_at, ai.ended_at, ru.username
