...
// Reload builds a new bridge from the provided providers and optional MCP proxy,
// swaps it in atomically, then gracefully shuts down the previous bridge with a 30-second timeout.
// mcpProxy may be nil when no MCP servers are configured.
func (bm *BridgeManager) Reload(providers []aibridge.Provider, mcpProxy aibmcp.ServerProxier) error {
	if len(providers) == 0 {
	...
	if mcpProxy != nil {
		if err := mcpProxy.Init(bm.ctx); err != nil {
			bm.logger.Warn(bm.ctx, "mcp proxy init warning", slog.Error(err))
		}
	}

	newBridge, err := aibridge.NewRequestBridge(bm.ctx, providers, bm.recorder, mcpProxy, bm.logger, bm.metrics, bm.tracer)
	if err != nil {
